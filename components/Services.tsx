...
// === SERVICES LIST - Uses i18n keys and ServiceType enum ===
const getServicesList = (t: any) => [
  { 
    icon: Plane, 
    type: ServiceType.AirFreight,
    title: t('services.airFreight.title'), 
    desc: t('services.airFreight.desc'), 
    color: "from-sky-400 to-blue-600" 
  },
  { 
    icon: Ship, 
    type: ServiceType.SeaFreight,
    title: t('services.seaFreight.title'), 
    desc: t('services.seaFreight.desc'), 
    color: "from-cyan-400 to-teal-600" 
  },
  { 
    icon: Truck, 
    type: ServiceType.LandFreight,
    title: t('services.landFreight.title'), 
    desc: t('services.landFreight.desc'), 
    color: "from-amber-400 to-orange-600" 
  },
  { 
    icon: DoorOpen, 
    type: ServiceType.DoorToDoor,
    title: t('services.doorToDoor.title'), 
    desc: t('services.doorToDoor.desc'), 
    color: "from-violet-400 to-purple-600" 
  },
  { 
    icon: Warehouse, 
    type: ServiceType.WarehouseStorage,
    title: t('services.warehouseStorage.title'), 
    desc: t('services.warehouseStorage.desc'), 
    color: "from-slate-400 to-slate-600" 
  },
  { 
    icon: PlaneTakeoff, 
    type: ServiceType.AirCharter,
    title: t('services.airCharter.title'), 
    desc: t('services.airCharter.desc'), 
  ...
  { 
    ...
    desc: t('services.procurement.desc'), 
    color: "from-lime-400 to-green-600" 
  },
  { 
    icon: Home, 
    type: ServiceType.HomeMoving,
    title: t('services.homeMoving.title'), 
    desc: t('services.homeMoving.desc'), 
  ...
  return (
    <section id="services" ref={containerRef} className="py-20 md:py-32 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* Gradient Orbs - RTL aware positioning */}
        <motion.div 
          style={{ y: backgroundY }}
          className={`absolute -top-40 ${isRTL ? '-left-40' : '-right-40'} w-[600px] h-[600px] bg-brand-500/10 rounded-full blur-3xl`}
        />
        <motion.div 
          style={{ y: backgroundY }}
          className={`absolute -bottom-40 ${isRTL ? '-right-40' : '-left-40'} w-[500px] h-[500px] bg-blue-500/10 rounded-full blur-3xl`}
        />
                ...
                <div className="relative z-10">
                  ...
                  <div className={`flex justify-between items-start mb-5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <motion.div 
                      ...
                      whileHover={{ rotate: isRTL ? 6 : -6, scale: 1.1 }}
                    ...
                    >
                      <ArrowUpRight size={14} className={`text-slate-400 group-hover:text-white transition-colors ${isRTL ? 'rotate-[270deg]' : ''}`} />
                    </motion.div>
                  </div>
                  
                  {/* Title */}
                  <h4 className={`text-lg font-bold text-white mb-2 group-hover:text-brand-400 transition-colors duration-300 ${isRTL ? 'text-right' : ''}`}>
                    {service.title}
                  </h4>
                  
                  {/* Description */}
                  <p className={`text-slate-400 text-sm leading-relaxed group-hover:text-slate-300 transition-colors duration-300 ${isRTL ? 'text-right' : ''}`}>
                    {service.desc}
                  </p>

                  {/* View Details text */}
                  <div className={`mt-4 flex items-center gap-1 text-brand-400 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <span>{t('common.viewDetails')}</span>
                    <ArrowUpRight size={14} className={isRTL ? 'rotate-[270deg]' : ''} />
                  </div>
                </div>

                {/* Hover glow effect */}
                <div className={`absolute -bottom-20 ${isRTL ? '-left-20' : '-right-20'} w-40 h-40 bg-gradient-to-br ${service.color} rounded-full blur-3xl opacity-0 group-hover:opacity-30 transition-opacity duration-500`} />
              </div>
            </motion.div>
          ))}
        </motion.div>
